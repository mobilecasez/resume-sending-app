...
app.get('/api/download-cover-letter/:filename', authenticateToken, async (req, res) => {
    try {
        // ⚠️ PATH TRAVERSAL. `filename` is user input and goes straight into path.join, which
        // RESOLVES `..` — so `..%2F..%2Fanything` escapes temp/ and serves an arbitrary file from
        // the container to any authenticated user. Reproduced against this repo's own express.
        // path.basename strips every directory component; comparing it back to the raw value
        // rejects the request rather than silently serving a different file. Every legitimate
        // filename here is generated by us (Name_Resume_<timestamp>.pdf) and already survives it.
