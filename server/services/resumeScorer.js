...
function buildPrompt(text, source) {
  return `You are a veteran technical recruiter who has screened tens of thousands of résumés.
Score the résumé below and return ONE raw JSON object — no markdown, no code fences, no commentary.

Scoring must be HONEST and CALIBRATED. Most real résumés land between 45 and 75. Reserve 85+ for a
résumé that would genuinely pass a top-tier screen with no changes. Do not inflate to be kind.

Judge these four dimensions, each 0-100:
...
{
  "score": <integer 0-100, your overall judgement — NOT a plain average>,
  "subscores": { "impact": <int>, "clarity": <int>, "keywords": <int>, "completeness": <int> },
  "headline": "<max 60 chars, the single biggest thing holding it back>",
  "summary": "<2 short sentences, max 220 chars total, plain language, why it scores what it does>",
  "improvements": [
    { "title": "<max 42 chars, an action>", "detail": "<max 110 chars, concretely what to change>" }
  ]
}
"improvements" must contain EXACTLY 3 items, ordered by how much each would raise the score.

=== RÉSUMÉ (${source === 'builder' ? 'structured JSON from our résumé builder' : 'text extracted from an uploaded file'}) ===
${text}`;
}
