    ...
    try {
        ...
        if (useAsync) {
            ...
            const jobId = await jobService.createJob(userId, 'generate_cover_letter', {
                recipientEmail, websiteUrl, position, responsibilities, jobLocation
            });
            ...
            const result = await executeGenerationWork(userId, user, { recipientEmail, websiteUrl, position, responsibilities, jobLocation });
 ...
 */
async function executeGenerationWork(userId, user, { recipientEmail, websiteUrl, position, responsibilities = null, jobLocation = null }) {
    console.log(`🚀 [executeGenerationWork] ENTERED — userId=${userId}, websiteUrl=${websiteUrl}, position=${position}, hasResponsibilities=${!!(responsibilities && responsibilities.length)}, jobLocation=${jobLocation || 'none'}`);
    // Normalize URL
    ...
    if (cached) {
        ...
        aiResult = await generateCoverLetterV2(resumeMetadata, normalizedWebsiteUrl, position, responsibilities, jobLocation);
        ...
        const [clResult, researchData] = await Promise.all([
            generateCoverLetterV2(resumeMetadata, normalizedWebsiteUrl, position, responsibilities, jobLocation),
